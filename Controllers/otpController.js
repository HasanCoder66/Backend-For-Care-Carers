...
export const sendOtp = async(req, res , next ) => {
    ...
    const sendOTPByEmail = async (toEmail, otp) => {
        try {
            let transporter = nodemailer.createTransport({
                service: 'Gmail',
                auth: {
                    user: 'codermhasan@gmail.com', // Sender's email address
                    pass: '3866coder.m.hasan' // Sender's email password
                }
            });
    
            let info = await transporter.sendMail({
                from: 'YOUR_EMAIL_ADDRESS', // Sender's email address
    ...
    // Example usage
    const userEmail = 'muhammadhasan3866@gmail.com'; // User's email address
    const otp = generateOTP(); // Generate OTP
    sendOTPByEmail(userEmail, otp); // Send OTP to user's email address
}
